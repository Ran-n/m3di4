        ...
        if not hasattr(self, "instance"):
            ...
            if self.language not in self.supported_languages.keys():
                raise LanguageException(
                    f"Language not supported yet, try: {self.supported_languages}"
                )

            try:
                self.ui = UIEnum(self.ui)
            except ValueError:
                raise UserInterfaceException(
                    f"User Interface not supported, try: {[o.value for o in UI]}"
                )
            #
    ...
    def get_table_name(self, table_name: str) -> str:
        ...
        try:
            ...
            raise TableNameException(
                f'Table "{table_name}" does not exist in the config \
