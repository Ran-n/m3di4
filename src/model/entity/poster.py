...
class Poster(BaseEntity):
    ...
    table_name: str = field(init=False, repr=False, default=Config().get_table_name("Poster"))
    extension: Extension
    name: str
    ...
    def __post_init__(self) -> None:
        ...
        if self.hash_ is None:
            self.hash_ = file_hash(Config().poster_folder / f"{self.name}.{self.extension.name}")
