...
class TrackLanguageDao:
    ...
    def save(self, track_language: TrackLanguage) -> TrackLanguage:
        ...
        if track_language.language.name is not None:
            is_code = self.model.exists(
                LanguageCode(language=None, code=None, codename=track_language.language.name)
            )
            is_name = self.model.exists(
                LanguageName(language=None, name=track_language.language.name)
            )

            if not is_code and not is_name:
                raise UnknownLanguageException(codename=track_language.language.name)
            elif is_code:
                track_language.language = self.model.get_language_by_codename(
