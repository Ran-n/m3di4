...
import os
try:
    import readline  # noqa: F401 — enables arrow-key/history in input() on Linux
except ImportError:
...
class Terminal(iView):
    ...
    def __show_menu(self, options: dict) -> int:
        ...
        title = center(
            f"{Config().title_symbol * 3} " + _("MENU") + f" {Config().title_symbol * 3}",
            self.line_len,
        )
        print()
        print("█" + "▀" * self.line_len + "█")
        ...
        for key, value in options.items():
            ...
            print(
                "▌"
                + center(
                    f"\t{key:<{biggest_key_len}}\t\t▐\t\t{value0:<{biggest_value_len}}\t".expandtabs(
                        self.tab_len
                    ),
                    self.line_len,
                )
    ...
    def save(self) -> None:
        ...
        print(self.separator)
        text = (
            f"{Config().title_symbol} " + _("The Database was saved") + f" {Config().title_symbol}"
        )
    ...
    def update_member_count(self) -> None:
        ...
        print(self.separator)
        text = (
            f"{Config().title_symbol} "
            + _("Searching for the values")
            + f" {Config().title_symbol}"
        )
        print(center(text, self.line_len))
        print(self.separator)
    ...
    def __pick_from_options(
        self,
        message: dict[str, str],
        option_count: Union[int, Callable],
        add_fn: Callable,
        get_opts_fn: Callable,
        limit: int = None,
        offset: int = 0,
        show_all: bool = False,
    ...
    def __pick_from_joined_options(
        self,
        message: dict[str, str],
        option_count: Union[int, Callable],
        add_fn: Callable,
        ...
        value = self.__pick_from_options(
            message=message,
            option_count=option_count,
            add_fn=add_fn,
        ...
        if value is None:
            ...
            get_opts_fn = lambda limit, offset: self.model.get_all(
                table_name=base_table, limit=limit, offset=offset
            )
            value = self.__pick_from_options(
                message=message,
                option_count=option_count,
    ...
    def __yn_question(message: str, as_str: bool = False) -> Union[str, int]:
        ...
        if user_input.lower() in no_opts:
            answer = 0

        if as_str:
            return str_answer[answer]
        return answer
    ...
    def __pick_date(
        ...
        nullable: bool = False,
        equal_to: str = None,
        restrictions: List[List[str]] = None,
    ) -> str:
        """
        restrictions
        ...
        while True:
            ...
            if any(
                [
                    self.__is_valid_date(date=date, date_format=date_format)
    ...
    def add_status(self) -> Status:
        ...
        ender = (
            f"{2 * Config().title_symbol} " + _("Added Status") + f" {2 * Config().title_symbol}"
        )
        print()
        print(self.separator)
        print(center(title, self.line_len))
    ...
    def add_media(self) -> Media:
        ...
        ender = (
            f"{2 * Config().title_symbol} " + _("Added Media") + f" {2 * Config().title_symbol}"
        )
        ...
        type_ = self.__pick_from_joined_options(
            ...
            option_count=lambda: self.model.get_num(
                table_name=Type.table_name
            ),
            add_fn=self.controller.add_type,
            get_opts_fn=lambda limit, offset: self.model.get_all(
                table_name=Type.table_name, limit=limit, offset=offset
            ),
        ...
        date_start = self.__pick_date(
            message=_("Start Date"), date_formats=["%Y-%m-%d", "%Y"], nullable=True
        )
        print()

        if self.model.exists(Media(name=name, type_=type_, status=status, date_start=date_start)):
        ...
        date_end = self.__pick_date(
            ...
            restrictions=[
                [
                    ">=",
                    date_start,
                    _(f"The end date must be equal or bigger than the start one ({date_start})"),
                ],
            ],
        )
        print()
        ...
        return Media(
            name=name, type_=type_, status=status, date_start=date_start, date_end=date_end
        )
    ...
    def add_group(self, id_media: int) -> Group:
        ...
        while True:
                ...
                compare_msg=[
                    {
                        "symbol": ">=",
                        "number": "0",
                        "message": "The group number must be bigger than 0",
                    }
                ],
            )
            ...
            if not self.model.exists(Group(media=media, number=number)):
                ...
                logging.info(
                    _(
                        "The requested media group to be added already exists, a number change will be adviced"
                    )
                )
                print(
                    f"{Config().error_symbol} "
                    + _("The Media Group already exists, pick another number.")
                )
        print()
        ...
        date_start = self.__pick_date(
            ...
            restrictions=[
                [
                    ...
                    media.date_start,
                    _(
                        f"The start date must be equal or bigger than the start date of its Media ({media.date_start})"
                    ),
                ],
                [
                    "<=",
                    media.date_end,
                    _(
                        f"The start date must be equal or smaller than the end date of its Media ({media.date_end})"
                    ),
                ],
            ],
        )
        ...
        date_end = self.__pick_date(
            ...
            restrictions=[
                ...
                [
                    ...
                    media.date_start,
                    _(
                        f"The end date must be equal or bigger than the start date of its Media ({date_start})"
                    ),
                ],
                [
                    "<=",
                    media.date_end,
                    _(
                        f"The end date must be equal or smaller than the end date of its Media ({media.date_end})"
                    ),
                ],
            ],
        )
        ...
        return Group(
            media=media, number=number, name=name, date_start=date_start, date_end=date_end
        )
    ...
    def add_issue(self) -> Issue:
        ...
        group = self.__pick_from_options(
            ...
            get_opts_fn=lambda limit, offset: self.model.get_group_by_media_id(
                id_=media.id_, limit=limit, offset=offset
            ),
            limit=Config().pagination_limit,
        )
        print()
        ...
        while True:
                ...
                compare_msg=[
                    {
                        "symbol": ">=",
                        "number": "0",
                        "message": "The issue number must be bigger than 0",
                    }
                ],
            )
            ...
            if not self.model.exists(Issue(media=media, group=group, position=position)):
                ...
                logging.info(
                    _(
                        "The requested media issue to be added already exists, a number change will be adviced"
                    )
                )
                print(
                    f"{Config().error_symbol} "
                    + _("The Media Issue already exists, pick another number.")
                )
        print()
        ...
        # date
        date = self.__pick_date(message=_("Date"), nullable=True)
        ...
        return Issue(position=position, media=media, group=group, name=name, date=date)
    ...
    def add_platform(self) -> Platform:
        ...
        title = (
            f"{2 * Config().title_symbol} " + _("Add Platform") + f" {2 * Config().title_symbol}"
        )
        ender = (
            f"{2 * Config().title_symbol} " + _("Added Platform") + f" {2 * Config().title_symbol}"
        )
        print()
        print(self.separator)
        print(center(title, self.line_len))
        ...
        while True:
            ...
            if name != "":
                if self.model.exists(Platform(name=name, link=None)):
                    print(
                        f"{Config().error_symbol} "
                        + _("The platform already exists, pick another name.")
                    )
                else:
                    break
        print()
        ...
        while True:
            ...
            if link != "" and valid.url(link):
                if not self.model.exists(ShareSite(name=name, type_=None, link=link)):
                    ...
                    logging.info(
                        _(
                            "The requested Platform to be added already exists, a link change will be adviced"
                        )
                    )
                    print(
                        f"{Config().error_symbol} "
                        + _("The Platform already exists, pick another link.")
                    )
        print()
        ...
        type_ = self.__pick_from_joined_options(
            ...
            option_count=lambda: self.model.get_num(
                table_name=(Type.table_name, Platform.table_name)
            ),
            add_fn=self.controller.add_type,
            get_opts_fn=lambda limit, offset: self.model.get_all(
        ...
        return Platform(name=name, name_long=name_long, acronym=acronym, link=link, type_=type_)

    def add_sharesite(self) -> ShareSite:
        """ """
        logging.info(_("Requesting the user for the information on the ShareSite"))
        title = (
            f"{2 * Config().title_symbol} " + _("Add ShareSite") + f" {2 * Config().title_symbol}"
        )
        ender = (
            f"{2 * Config().title_symbol} "
            + _("Added ShareSite")
        ...
        while True:
            ...
            if link != "" and valid.url(link):
                if not self.model.exists(ShareSite(name=name, type_=None, link=link)):
                    ...
                    logging.info(
                        _(
                            "The requested ShareSite to be added already exists, a link change will be adviced"
                        )
                    )
                    print(
                        f"{Config().error_symbol} "
                        + _("The ShareSite already exists, pick another link.")
                    )
        print()
        ...
        type_ = self.__pick_from_joined_options(
            ...
            option_count=lambda: self.model.get_num(
                table_name=(Type.table_name, ShareSite.table_name)
            ),
            add_fn=self.controller.add_type,
            get_opts_fn=lambda limit, offset: self.model.get_all(
    ...
    def add_warehouse(self) -> Warehouse:
        ...
        title = (
            f"{2 * Config().title_symbol} " + _("Add Warehouse") + f" {2 * Config().title_symbol}"
        )
        ender = (
            f"{2 * Config().title_symbol} "
            + _("Added Warehouse")
        ...
        type_ = self.__pick_from_joined_options(
            ...
            option_count=lambda: self.model.get_num(
                table_name=(Type.table_name, Warehouse.table_name)
            ),
            add_fn=self.controller.add_type,
            get_opts_fn=lambda limit, offset: self.model.get_all(
            ...
            compare_msg=[
                {
                    "symbol": ">=",
                    "number": "0",
                    "message": "The filled number must be bigger than 0",
                }
            ],
        )
        print()
        ...
        return Warehouse(
            name=name, type_=type_, size=size, filled=filled, content=content, health=health
        )
    ...
    def add_file(self) -> AddFileTerminalViewOutput:
        ...
        while True:
            file_path = os.path.expanduser(
                input(f"{Config().input_symbol} " + _("File Path") + ": ")
            )
        ...
        if is_media == 1:
            ...
            output_obj = AddFileTerminalViewOutput(
    ...
    def add_media_platform(self) -> MediaPlatform:
        ...
        return MediaPlatform(
            media=media, platform=platform, link=link, in_platform_id=in_platform_id
        )
